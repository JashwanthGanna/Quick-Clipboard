...
import {
  Calendar, Clock, ArrowLeft, ArrowRight, User, Share2, BookOpen,
} from "lucide-react";
...
function renderMarkdown(content: string) {
  ...
  const flushList = () => {
    if (currentList.length > 0) {
      elements.push(
        <ul key={`list-${key++}`} className="list-disc list-inside text-slate-400 space-y-1 ml-4 mb-4">
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("## ")) {
      ...
      elements.push(
        <p key={key++} className="text-slate-400 leading-relaxed mb-4">
          {parts.map((part, j) =>
            j % 2 === 1 ? (
              <strong key={j} className="text-slate-300 font-semibold">{part}</strong>
            ) : (
              part
            )
          )}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900/50 to-slate-900">
      ...
      <section className="pt-28 lg:pt-36 pb-8">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/blog" className="inline-flex items-center text-sm text-purple-400 hover:text-purple-300 mb-6 transition-colors">
            <ArrowLeft className="w-4 h-4 mr-1" /> Back to Blog
          </Link>

          <div className="mb-6">
            <span className="inline-block bg-purple-500/20 text-purple-300 text-xs font-semibold px-3 py-1 rounded-full mb-4">
              {post.category}
            </span>
      ...
      <article className="pb-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-8 lg:p-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {relatedPosts.map((related) => (
              <Link key={related.slug} href={`/blog/${related.slug}`}>
                <Card className="group backdrop-blur-xl bg-white/5 border border-white/10 hover:border-purple-500/30 transition-all duration-300 p-6 cursor-pointer">
                  ...
                  <p className="text-sm text-slate-400 line-clamp-2 mb-3">{related.excerpt}</p>
